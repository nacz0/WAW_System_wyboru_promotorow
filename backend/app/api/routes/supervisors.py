...
def create_supervisor(payload: SupervisorCreate, db: Session = Depends(get_db)) -> Supervisor:
...
def list_supervisors(db: Session = Depends(get_db)) -> list[Supervisor]:
...
def get_supervisor(supervisor_id: int, db: Session = Depends(get_db)) -> Supervisor:
...
def update_supervisor(supervisor_id: int, payload: SupervisorUpdate, db: Session = Depends(get_db)) -> Supervisor:
    supervisor = db.get(Supervisor, supervisor_id)
    if supervisor is None:
        raise HTTPException(status_code=404, detail="supervisor not found")
...
def create_topic(supervisor_id: int, payload: TopicProposalCreate, db: Session = Depends(get_db)) -> TopicProposal:
...
def list_topics(supervisor_id: int, db: Session = Depends(get_db)) -> list[TopicProposal]:
    if db.get(Supervisor, supervisor_id) is None:
        raise HTTPException(status_code=404, detail="supervisor not found")
    return list(
        db.execute(
